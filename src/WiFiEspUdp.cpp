...
//        EspDrv::startClient("0", port, sock, UDP_MODE);
		esp32_i2c_socket_open(sock, (uint8_t *)"0", 1, port, UDP_MODE);
...
int WiFiEspUDP::beginPacket(const char *host, uint16_t port)
{
  ...
  if (_sock != NO_SOCKET_AVAIL)
  {
	  ...
	  esp32_i2c_socket_connect(_sock, (uint8_t *)host, 1, port, UDP_MODE);
	  _remotePort = port;
...
//	return beginPacket(s, port);
  ...
  if (_sock != NO_SOCKET_AVAIL)
  {
	  ...
	  _ip[2] = ip[2];
	  _ip[3] = ip[3];
	  esp32_i2c_socket_connect(_sock, _ip, 0, port, UDP_MODE);
	  _remotePort = port;
...
//	bool r = EspDrv::sendDataUdp(_sock, _remoteHost, _remotePort, buffer, size);
	int8_t r = esp32_i2c_add_udp_data(_sock, (uint8_t *)buffer, size);
	r = esp32_i2c_send_udp_data(_sock);
	if (!r)
	{
		return 0;
	}
...
uint8_t WiFiEspUDP::beginMulticast(IPAddress ip, uint16_t port)
{
  ...
  if (_sock != NO_SOCKET_AVAIL)
  {
	  ...
	  _ip[2] = ip[2];
	  _ip[3] = ip[3];
	  esp32_i2c_start_server(_sock, _ip, 0, port, UDP_MODE_2);
	  _remotePort = port;
	  memcpy(_remoteHost, _ip, 4);
	  WiFiEspClass::allocateSocket(_sock);
	  return 1;
