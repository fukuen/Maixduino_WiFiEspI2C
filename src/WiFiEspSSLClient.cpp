...
int WiFiEspSSLClient::connect(const char* host, uint16_t port, uint8_t protMode)
{
    ...
    {
		if (esp32_i2c_socket_connect(_sock, (uint8_t *)host, 1, port, (esp32_socket_mode_enum_t)protMode))
			return 0;
...
size_t WiFiEspSSLClient::write(const uint8_t *buf, size_t size)
{
	...
	uint32_t r = esp32_i2c_socket_write(_sock, (uint8_t *)buf, size);
	if (!r)
	{
		setWriteError();
...
int WiFiEspSSLClient::available()
{
	if (_sock != 255)
	{
		int bytes = esp32_i2c_socket_available(_sock);
		if (bytes>0)
		{
			return bytes;
		}
...
int WiFiEspSSLClient::read(uint8_t* buf, size_t size)
{
	if (!available())
		return -1;
	return esp32_i2c_socket_read(_sock, buf, size);
}
...
void WiFiEspSSLClient::setCACert(const char *root_ca)
{
	esp32_set_ca_certificate((char *)root_ca);
}
...
//	LOGINFO1(F("SOCKET_CLOSED 1! "), _sock);
	...
	if (esp32_i2c_socket_available(_sock) == 0)
	{
//	LOGINFO1(F("SOCKET_ESTABLISHED 1! "), _sock);
		return SOCKET_ESTABLISHED;
	}

	if (esp32_i2c_socket_status(_sock) == SOCKET_ESTABLISHED)
	{
...
IPAddress WiFiEspSSLClient::remoteIP()
{
	...
	uint16_t port = 0;
	uint16_t *p = &port;
	esp32_i2c_get_remote_info(_sock, ip, p);
	ret = ip;
	return ret;
}
...
size_t WiFiEspSSLClient::printFSH(const __FlashStringHelper *ifsh, bool appendCrLf)
{
	...
	uint32_t r = esp32_i2c_socket_write(_sock, (uint8_t *)ifsh, size);
	if (!r)
	{
		setWriteError();
		LOGERROR1(F("Failed to write to socket"), _sock);
